...
void Centre::supprimerReservation()
{
	...
	cin >> i;
	Date x = reservation[i].service[0]->dispo[0].date;
	Temps y = reservation[i].service[0]->dispo[0].temps;
	for (int j = 0; j < salles.size(); j++)
	{
		for (int k = 0; k < salles[j].dispo.size(); k++)
		{
			if (salles[j].dispo[k].date == x && salles[j].dispo[k].temps == y)
			{
				salles[j].dispo.erase(salles[j].dispo.begin() + k);
			}
		}
...
{
	f.open("C:\\Users\\Ezer\\FichierSalle.txt", ios::in | ios::out | ios::trunc);
	if (!f.is_open()) exit(-1);
}
